...
- マニュアル/イベントは実データ表示に更新（`/api/content/manuals`, `/api/content/events`）
...
- 2026-03-08: v1.10 更新（ユーザー画面の状態表示を改善: 状態説明、最終更新表示、手動再取得導線を追加）
- 2026-03-08: v1.11 更新（E2Eチェックリストを追加し、運用検証手順を定義）
- 2026-03-08: v1.12 更新（監査ログ活用指標を定義し、週次KPIと異常検知条件を追加）
- 2026-03-08: v1.13 更新（ログイン後のマニュアル/イベントを実データ表示へ変更）
- 2026-03-08: v1.14 更新（週次モニタリングレポートと閾値超過時テンプレートを追加）
- 2026-03-08: v1.15 更新（週次モニタリングの初回2週間実測レポートを追加）
...
予定タスク:
- コンテンツの管理元を静的データからCMSまたは管理画面更新へ移行

### 8.3 iOS対応の検討

予定タスク:
- iOS申請機能を追加するか、案内表示のまま維持するかを決定
- 追加する場合はAndroidと同様の手動オペレーション待ちキューフローを定義

備考:
- 2026-03-08時点では優先度判断により一旦スキップ
